...
def update_product(db: Session, asin: str, product_data: dict) -> Optional[models.Product]:
...
def upsert_product(db: Session, product_data: dict) -> models.Product:
    """Create product if not exists, update if exists"""
    asin = product_data.get('asin')
    existing = get_product_by_asin(db, asin)
    
    if existing:
        return update_product(db, asin, product_data)
    else:
...
def create_price_history(db: Session, price_data: dict) -> models.PriceHistory:
